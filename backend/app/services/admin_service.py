...
class AdminService:
    ...
    @staticmethod
    async def get_coefficients_by_bank(db: AsyncSession, bank_id: int):
        result = await db.execute(select(Coefficient).where(Coefficient.bank_id == bank_id))
        return result.scalars().all()

    @staticmethod
    async def create_coefficient(db: AsyncSession, coeff: CoefficientCreate):
        # Evitar duplicados: verificar se já existe o mesmo banco, tabela e prazo
        result = await db.execute(
            select(Coefficient).where(
                Coefficient.bank_id == coeff.bank_id,
                Coefficient.table_id == coeff.table_id,
                Coefficient.term == coeff.term,
                Coefficient.interest_rate == coeff.interest_rate
            )
        )
        existing = result.scalar_one_or_none()
        
        if existing:
            existing.coefficient = coeff.coefficient
            if coeff.interest_rate_refin is not None:
                existing.interest_rate_refin = coeff.interest_rate_refin
            await db.commit()
    ...
    async def update_user(db: AsyncSession, user_id: int, user_data: dict, current_user: User):
        ...
        if current_user.role == "promotora":
            ...
            restricted_fields = ["brand_color", "sidebar_color", "sidebar_color_secondary", "logo_url", "role", "broker_id", "seller_limit", "can_consult_cpf"]
            for field in restricted_fields:
                if field in user_data:
                    user_data.pop(field)
    ...
    _dashboard_cache = {}
    _dashboard_cache_ttl = 300 # Cache de 5 minutos
    _dashboard_locks = {}
    
    _global_logos_cache = None
    _global_logos_timestamp = 0
    ...
    async def get_dashboard_stats(db: AsyncSession, current_user: User, days: int = 30):
        ...
        if cache_key not in AdminService._dashboard_locks:
            AdminService._dashboard_locks[cache_key] = asyncio.Lock()
            
        async with AdminService._dashboard_locks[cache_key]:
        ...
        # Execute queries sequentially on the single session db connection to avoid exceeding PgBouncer pool limits
        try:
            counts_res = (await db.execute(counts_q)).all()
        except Exception as e:
            print(f"[ERROR] Query counts falhou - {e}")
            counts_res = []

        try:
            origin_counts = (await db.execute(origin_counts_q)).all()
        except Exception as e:
            print(f"[ERROR] Query origin_counts falhou - {e}")
            origin_counts = []

        try:
            agreement_counts = (await db.execute(agreement_counts_q)).all()
        except Exception as e:
            print(f"[ERROR] Query agreement_counts falhou - {e}")
            agreement_counts = []

        try:
            user_counts = (await db.execute(user_counts_q)).all()
        except Exception as e:
            print(f"[ERROR] Query user_counts falhou - {e}")
            user_counts = []

        try:
            historical_raw = (await db.execute(historical_q)).all()
        except Exception as e:
            print(f"[ERROR] Query historical falhou - {e}")
            historical_raw = []

        try:
            bank_stats = (await db.execute(bank_stats_q)).all()
        except Exception as e:
            print(f"[ERROR] Query bank_stats falhou - {e}")
            bank_stats = []

        try:
            table_stats = (await db.execute(table_stats_q)).all()
        except Exception as e:
            print(f"[ERROR] Query table_stats falhou - {e}")
            table_stats = []

        try:
            avg_rate = (await db.execute(rate_stats_q)).scalar() or 0
        except Exception as e:
            print(f"[ERROR] Query rate_stats falhou - {e}")
            avg_rate = 0

        try:
            hist_val = (await db.execute(hist_val_q)).all()
        except Exception as e:
        ...
        total_tables = counts_row[1] if counts_row else 0
        total_simulations = counts_row[2] if counts_row else 0
        sim_period_count = counts_row[3] if counts_row else 0
        
        # Load heavy logos/banks only if cache expired (1 hour TTL)
        if not AdminService._global_logos_cache or now - AdminService._global_logos_timestamp > AdminService._global_logos_ttl:
            try:
                all_banks = (await db.execute(banks_q)).scalars().all()
            except Exception as e:
                print(f"[ERROR] Query banks_q falhou - {e}")
                all_banks = []

            try:
                sub_logos = (await db.execute(sub_logos_q)).scalars().all()
            except Exception as e:
                print(f"[ERROR] Query sub_logos_q falhou - {e}")
                sub_logos = []

            AdminService._global_logos_cache = {
            ...
            AdminService._global_logos_timestamp = now
            
        banks_map = AdminService._global_logos_cache["banks_map"]
        banks_logo_map = AdminService._global_logos_cache["banks_logo_map"]
        sub_logos_map = AdminService._global_logos_cache["sub_logos_map"]
