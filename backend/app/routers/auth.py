...
async def login(req: schemas.LoginRequest, db: AsyncSession = Depends(get_db)):
    try:
        ...
        print(f"ERRO FATAL NO LOGIN: {error_details}")
...
async def update_user_meta(
    ...
    current_user: sqlalchemy_models.User = Depends(get_current_user)
):
    from app.routers.deps import get_current_user
    current_user.monthly_goal = float(meta_data.get("valor_alvo", 110000.0))
    current_user.daily_goal = float(meta_data.get("valor_diario", 5000.0))
    current_user.monthly_goal_type = str(meta_data.get("tipo", "mensal"))
    
    await db.commit()
    await db.refresh(current_user)
    
    return {
        "tipo": current_user.monthly_goal_type,
        "valor_diario": current_user.daily_goal,
        "valor_alvo": current_user.monthly_goal
    }
