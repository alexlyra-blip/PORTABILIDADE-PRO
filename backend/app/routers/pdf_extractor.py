...
class PdfExtractorService:
    ...
    async def extract_pdf(pdf_bytes: bytes) -> dict:
        # Simplest mock that returns success so the chat can acknowledge it
...
async def extract_inss_pdf(file: UploadFile = File(...)):
    ...
    try:
        ...
        with pdfplumber.open(
            ...
            for page in detection_pdf.pages[:3]:
                try:
        ...
        if detectar_extrato_siape(all_text):
            siape_data = parse_siape_extrato(all_text)

            # Nunca utiliza o coeficiente INSS em extratos SIAPE.
            siape_data["valor_liberado_margem"] = 0.0
        ...
        # preserva integralmente o leitor INSS existente.
        def parse_inss_sync():
            ...
            with pdfplumber.open(pdf_stream) as pdf:
                    ...
                    for i, line in enumerate(lines):
                        ...
                        if "BLOQUEADO PARA EMPR" in line.upper() or "BLOQUEADO PARA EMPRESTIMO" in line.upper():
                            if "SIM" in line.upper():
                                ...
                                extracted_data["bloqueado_emprestimo"] = True
                        elif "LIBERADO PARA EMPR" in line.upper() or "ELEGÍVEL PARA EMPR" in line.upper() or "ELEGIVEL PARA EMPR" in line.upper():
                            if extracted_data.get("bloqueado_emprestimo") is None or extracted_data.get("bloqueado_emprestimo") is False:
                                extracted_data["bloqueado_emprestimo"] = False
                                ...
                                for idx_cell, cell in enumerate(clean_row):
                                    if "BLOQUEADO PARA EMPR" in cell or "BLOQUEADO PARA EMPRESTIMO" in cell:
                                        if idx_cell + 1 < len(clean_row):
                                            ...
                                            extracted_data["bloqueado_emprestimo"] = True
                                    elif "LIBERADO PARA EMPR" in cell or "ELEGÍVEL PARA EMPR" in cell or "ELEGIVEL PARA EMPR" in cell:
                                        if extracted_data.get("bloqueado_emprestimo") is None or extracted_data.get("bloqueado_emprestimo") is False:
                                            extracted_data["bloqueado_emprestimo"] = False
                ...
                for page_index, page in enumerate(pdf.pages):
                    ...
                    if (
                        page_index > 1
                        and "CONTRATOS EXCLUÍDOS E ENCERRADOS" in page_upper
                    ):
                        break
                    ...
                    if "CONTRATOS ATIVOS E SUSPENSOS" not in page_upper:
                        continue
        ...
        extracted_data["valor_liberado_margem"] = await calcular_valor_liberado_margem(extracted_data["margem_disponivel"])
