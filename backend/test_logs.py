...
async def main():
    ...
    async with async_session() as db:
        try:
            query = select(WhatsappChatLog).order_by(WhatsappChatLog.created_at.desc()).limit(3)
            result = await db.execute(query)
            logs = result.scalars().all()
            
            for log in logs:
                print(f"\\n\\n=== PROTOCOL: {log.protocol} | PHONE: {log.sender_phone} ===")
                try:
                    msgs = json.loads(log.messages)
                    for m in msgs[-10:]:  # last 10 messages
                        print(f"[{m['role'].upper()}]: {m['text']}")
                except Exception as e:
                    print("Could not parse messages JSON")
