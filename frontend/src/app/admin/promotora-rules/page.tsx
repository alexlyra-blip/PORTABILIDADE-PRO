...
export default function PromotoraRulesPage() {
  ...
  // Blocked Tables State
  const [blockedTables, setBlockedTables] = useState<number[]>([]);
  const [selectedBankForTables, setSelectedBankForTables] = useState("");
  const [bankTables, setBankTables] = useState<any[]>([]);
  ...
  const loadAll = async (userId: number) => {
    try {
      ...
      const blockedTablesRule = rulesData.find((r: any) => r.rule_key === "blocked_tables");
      if (blockedTablesRule) setBlockedTables(JSON.parse(blockedTablesRule.rule_value));
  ...
  const saveBlockedTables = async (updated: number[]) => {
    if (!loggedUser) return;
    try {
  ...
  const loadBankTables = async (bankId: string) => {
    setSelectedBankForTables(bankId);
    if (!bankId) {
      setBankTables([]);
      return;
    }
  ...
  const toggleTableBlock = async (tableId: number) => {
    if (loadingTableId) return;
    setLoadingTableId(tableId);
    let updated = [...blockedTables];
    if (updated.includes(tableId)) {
      updated = updated.filter(id => id !== tableId);
    } else {
      updated.push(tableId);
    }
    await saveBlockedTables(updated);
    ...
    <div className="p-4 lg:p-8 space-y-8 bg-slate-50 min-h-screen">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
        ...
        <motion.div 
          ...
          className="bg-white rounded-[2.5rem] p-8 shadow-sm border border-slate-100 xl:col-span-2"
        >
          <div className="flex items-center gap-4 mb-6">
            <div className="w-12 h-12 bg-blue-600 rounded-2xl flex items-center justify-center text-white text-xl shadow-lg shadow-blue-200">
              <Icons.Table size={24} />
            ...
            <div>
              <h2 className="text-xl font-black text-slate-900 uppercase tracking-tight">Bloqueio de Tabelas por Banco</h2>
              <p className="text-xs text-slate-400 font-bold uppercase tracking-widest">Selecione um banco para visualizar e bloquear tabelas específicas de simulação</p>
            </div>
          </div>

          <div className="bg-slate-50 p-6 rounded-3xl border border-slate-100 mb-6">
            <div className="mb-4">
              <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 ml-1">Selecione o Banco</label>
              <select 
                value={selectedBankForTables}
                onChange={e => loadBankTables(e.target.value)}
                className="w-full h-12 px-4 rounded-xl border-2 border-slate-200 focus:border-blue-500 outline-none font-bold text-sm bg-white"
              >
                <option value="">Selecione um banco para listar as tabelas...</option>
                {banks.map(b => <option key={b.id} value={b.id}>{b.name}</option>)}
              </select>
            </div>
          </div>

          {selectedBankForTables && bankTables.length > 0 && (() => {
            const groupedTables = bankTables.reduce((acc, t) => {
              const agreement = t.agreement || 'Outros';
              if (!acc[agreement]) acc[agreement] = [];
              acc[agreement].push(t);
              return acc;
            }, {} as Record<string, any[]>);

            return (
              <div className="space-y-8 max-h-[500px] overflow-y-auto pr-2 custom-scrollbar">
                {Object.entries(groupedTables).map(([agreement, tables]) => (
                  <div key={agreement} className="bg-slate-50/50 p-6 rounded-3xl border border-slate-100">
                    <div className="flex items-center gap-3 mb-6 border-b border-slate-100 pb-4">
                      ...
                      <span className="ml-auto text-[10px] font-bold text-slate-400 bg-white px-3 py-1 rounded-full border border-slate-200 shadow-sm">
                        {(tables as any[]).length} tabelas
                      </span>
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                      {(tables as any[]).map((t: any) => {
                        const isBlocked = blockedTables.includes(t.id);
                        return (
                          <div 
                            key={t.id}
                            onClick={() => toggleTableBlock(t.id)}
                            className={`cursor-pointer p-4 rounded-2xl border-2 transition-all flex flex-col justify-between h-full relative ${
                              isBlocked 
                          ...
                          >
                            {loadingTableId === t.id && (
                               <div className="absolute inset-0 bg-white/60 dark:bg-slate-900/60 backdrop-blur-[2px] rounded-2xl flex flex-col items-center justify-center z-10">
                                  <Icons.RefreshCw size={24} className="text-blue-600 animate-spin mb-2" />
                                  <span className="text-[9px] font-black uppercase tracking-widest text-blue-700 animate-pulse">Atualizando...</span>
                               </div>
                            )}
                            <div>
                              <div className="flex items-start justify-between mb-2">
                                <h3 className={`font-black text-sm leading-tight ${isBlocked ? 'text-red-700' : 'text-slate-800'}`}>
                                  {t.name}
                                </h3>
                                {isBlocked ? (
                                  <div className="w-6 h-6 rounded-md bg-red-500 flex items-center justify-center text-white shrink-0">
                                    <Icons.Ban size={14} />
                                  </div>
                                ) : (
                              ...
                              <div className="space-y-1 mt-3">
                                 <p className={`text-[10px] font-bold uppercase tracking-widest ${isBlocked ? 'text-red-400' : 'text-slate-400'}`}>
                                   Clique para {isBlocked ? 'Desbloquear' : 'Bloquear'}
                                 </p>
                              </div>
                            </div>
                          </div>
          ...
          {selectedBankForTables && bankTables.length === 0 && (
            <p className="text-center py-8 text-slate-400 font-bold uppercase text-xs tracking-widest italic">Nenhuma tabela encontrada para este banco.</p>
          )}
        </motion.div>
      </div>
    </div>
