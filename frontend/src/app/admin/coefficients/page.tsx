      ...
      <div className="space-y-10">
        {loading && Object.keys(allCoefficients).length === 0 ? (
          ...
          displayedWindows.map((w) => {
            return (
              <div key={w.key} className="bg-white dark:bg-slate-900 rounded-[3.5rem] overflow-hidden border border-slate-100 dark:border-white/10 shadow-2xl animate-in slide-in-from-bottom-8 duration-700 mb-10">
                 <div className="px-10 py-8 bg-slate-50/50 dark:bg-white/5 border-b border-slate-100 dark:border-white/5 flex flex-col md:flex-row items-center gap-8">
                    ...
                    <div className="text-center md:text-left flex-1">
                       <h2 className="text-xl font-black text-slate-800 dark:text-white uppercase tracking-tighter mb-1 leading-tight flex flex-wrap items-center gap-3 justify-center md:justify-start">
                          <span>{selectedBank?.name} • {w.table.name}</span>
                          {w.term !== null && (
                            <span className="px-3.5 py-1.5 bg-blue-600 dark:bg-blue-500 text-white text-[10px] font-black uppercase rounded-xl tracking-widest shadow-md shadow-blue-500/20">
                               {w.term}X
                            </span>
                          )}
                       </h2>
                       <div className="flex flex-wrap items-center justify-center md:justify-start gap-3 mt-2">
                          <span className="px-3 py-1 bg-slate-900 text-white text-[9px] font-black uppercase rounded-lg tracking-widest">
                             CONVÊNIO: {w.table.agreement || "N/A"}
                          </span>
                          ...
                          <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">
                             Taxa Tabela: {w.table.taxa_convenio}%
                          </span>
                       </div>
                    </div>
                    <button 
                      onClick={() => handleOpenModal(null, w.table.id.toString(), w.term)}
                      className="py-3 px-8 bg-white dark:bg-slate-800 hover:bg-blue-600 hover:text-white text-slate-500 dark:text-white rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all border border-slate-200 dark:border-white/10 shadow-sm flex items-center gap-2 shrink-0 hover:-translate-y-0.5"
                    >
                      <span>＋</span> Novo Coeficiente
                    </button>
                 </div>
