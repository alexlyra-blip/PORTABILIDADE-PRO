...
export default function CoefficientsPage() {
  ...
  const loadBanks = async () => {
    try {
      const data = await api.get(`/admin/banks?_t=${Date.now()}`);
      setBanks(data);
  ...
  const loadTables = async (bankId: string) => {
    try {
      const data = await api.get(`/admin/banks/${bankId}/tables?_t=${Date.now()}`);
      setTables(data);
    } catch (error) {
  ...
  const loadAllCoefficients = async (tablesList: any[]) => {
    try {
      ...
      const bankId = tablesList[0].bank_id;
      const allData = await api.get(`/admin/banks/${bankId}/coefficients?_t=${Date.now()}`);
      
      // Organizar por table_id
      const coeffData: Record<string, any[]> = {};
      tablesList.forEach(t => {
        coeffData[t.id] = [];
      });
      
      allData.forEach((coeff: any) => {
        if (!coeffData[coeff.table_id]) {
          coeffData[coeff.table_id] = [];
        }
        coeffData[coeff.table_id].push(coeff);
      });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const payload = { 
        ...
        interest_rate: parseFloat(String(formData.interest_rate).replace(",", ".")),
        coefficient: String(formData.coefficient).replace(",", ".")
      };
    ...
    <div className="space-y-6 animate-fade-in pb-20">
      ...
      <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-4">
        <div className="flex flex-wrap items-center gap-3 w-full xl:w-full">
          ...
          >
            <Icons.Plus size={16} /> Novo Coeficiente
          </button>
        </div>
      </div>
            ...
            <form onSubmit={handleSubmit} className="p-8 space-y-6">
              ...
              <div className="grid grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <input type="text" required value={formData.interest_rate} onChange={(e) => setFormData({...formData, interest_rate: e.target.value})} className="input-admin !py-3" placeholder="1.77" />
                </div>
              </div>
              <div>
