...
async def executar_simulacao_completa(cliente_input, db: AsyncSession, user_id: int):
    ...
    try:
        ...
        for r in p_rules:
            ...
            if r.rule_key == 'blocked_tables': p_blocked_tables = json.loads(r.rule_value)
        ...
        for banco in todos_os_bancos:
            try:
                ...
                # FILTRO 3: Banco de origem igual ao banco destino (não permite portar pro mesmo banco)
                if full_origin_name and input_words:
                    banco_destino_words = get_clean_words(banco_name_upper)
                    is_same_bank = False
                    if len(banco_destino_words.intersection(input_words)) > 0:
                        is_same_bank = True
                    elif banco_name_upper in full_origin_name or full_origin_name in banco_name_upper:
                        is_same_bank = True
                    ...
                    if is_same_bank:
                        rejeitados.append({
                            ...
                            "bank_id": banco.id,
                            "logo_url": banco.logo_url,
                            "motivo": f"Portabilidade negada: o contrato já pertence a este banco ({full_origin_name}).",
                            "elegivel": False
                        })
                        continue
                ...
                for tabela in banco.tables:
                    ...
                    # A validação de Ticket Mínimo e Máximo agora é feita exclusivamente no financial_engine.py,
                    # pois lá temos o valor exato da 'capacidade de financiamento' (Saldo Devedor + Troco),
                    # que é o real 'Ticket' da operação de refinanciamento/portabilidade.
                    ...
                    for coeff_obj in tabela.coefficients:
                        ...
                        try:
                            # Nova interpretação: O usuário marca a flag no painel para ATIVAR a validação
                            should_enable_weighted = any(r.disable_weighted_rate_validation for r in regras_aplicaveis)
                            should_abate_hp12c = getattr(tabela, "abater_margem_hp12c", False)
                            
                            resultado = calcular_viabilidade_financeira(
                                cliente_input, banco, coeff_obj, tabela,
                                enable_weighted_validation=should_enable_weighted,
                    ...
                    if not tem_coeficiente_valido:
                        if not tabela.coefficients:
                            motivos_tabelas.append(f"Tabela {tabela.name}: Sem coeficientes cadastrados.")
                        else:
        ...
        # Ordena por prioridade do admin (se houver), depois prioridade da promotora (se houver), depois prioridade global do banco, e por fim MENOR valor liberado (maior taxa)
